...
RUN mkdir /var/lib/mysql \
    ...
    && dnf install -y which MariaDB-server-${mariadb_version} MariaDB-client-${mariadb_version} groonga-${groonga_version} groonga-tokenizer-mecab-${groonga_version} mariadb-10.4-mroonga-${mroonga_version} \
    && dnf clean all \
    && rm -rf /var/lib/mysql
